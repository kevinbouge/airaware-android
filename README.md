...
- Android home-screen widgets generated by a local Expo config plugin for development builds
...
- Android home-screen widgets:
  ...
  - Pro advanced widget with current score, best outdoor window, and forecast summaries
...
- **Today**: headline scores, location, update status, main factor, collapsible current-reading
  sections, Mold and sun, and Pro-only Atmospheric composition, Pressure and visibility, Clouds and
...
- **Profile**: local Personal Allergy Profile toggles, including Mold potential and UV index.
  Informational extended measurements remain display-only on Today
...
- **AirAware Pro advanced widget**: requires the advanced widget capability. It shows the active
  headline score, main factor, best outdoor window when available, and daily forecast summaries up
  to the active forecast horizon.
...
part of normal system behavior. Locked Pro widgets do not expose environmental data when Pro is
unavailable.
...
Google Play Billing remains inactive. Development builds may show Pro capabilities through the
existing protected local development entitlement path so the advanced widget can be tested before
billing exists.
...
capability profile for Extended Forecast, Extended Environmental Data, and the advanced home-screen
widget. Extended Forecast changes the daily forecast summary from 3 total days to 4 total days.
Extended Environmental Data exposes additional informational readings where the provider supports
them. Mold potential and UV index remain standard readings and optional Personal Allergy Profile
factors for Free and Pro users. These capabilities do not change the environmental burden formula,
cache behavior, location behavior, notifications, or sharing.
...
score and centralized forecast horizon as the app. Widget code consumes a presentation-safe snapshot
prepared by the app; it does not parse provider JSON or duplicate scoring logic.
...
- No background location, advanced environmental notifications, OpenStreetMap vegetation context,
  accounts, analytics, or long-term history are included in this first Android milestone.
- Google Play Billing is not integrated yet. The currently modeled Pro capabilities are one
  additional daily forecast summary day, Extended Environmental Data, and the advanced home-screen
  widget; advanced environmental notifications are capability metadata only.
